...
class Solution {
   public:
    ...
    // Optimised Approach using Boyer - Moore Voting Algorithm & Hash map
    // Time Complexity: O(n), Space Complexity: O(n) in worst case (if all elements are unique)
    // vector<int> majorityElement(vector<int>& nums) {
    ...
    // Optimised Approach using Boyer - Moore Voting Algorithm - Time Complexity: O(n), Space Complexity: O(1)
